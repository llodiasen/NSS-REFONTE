      ...
      >
        ...
        {active === "Notre histoire" && (
          <div className="tabs-panel" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "56px", alignItems: "center" }}>
            <div>
              <span style={{ fontFamily: "var(--font-body)", fontSize: "11px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "2px", color: "var(--green-600)", display: "block", marginBottom: "16px" }}>
                Depuis 2011
              </span>
              <h3 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(20px, 2vw, 26px)", fontWeight: 400, lineHeight: 1.25, color: "var(--text-primary)", marginBottom: "20px" }}>
                Née des champs, portée par des milliers de voix.
              </h3>
              <p style={{ fontFamily: "var(--font-body)", fontSize: "15px", lineHeight: 1.8, color: "var(--text-body)", marginBottom: "32px", textAlign: "justify" }}>
              ...
              <div style={{ display: "flex", gap: "32px", marginBottom: "28px" }}>
                {[{ val: "175 000", lbl: "Membres" }, { val: "500+", lbl: "AFR" }, { val: "2011", lbl: "Fondation" }].map(({ val, lbl }) => (
                  <div key={lbl}>
                    <div style={{ fontFamily: "var(--font-display)", fontSize: "26px", fontWeight: 400, color: "#1a6b3c", lineHeight: 1 }}>{val}</div>
                    <div style={{ fontFamily: "var(--font-body)", fontSize: "12px", color: "var(--text-muted)", marginTop: "4px" }}>{lbl}</div>
                  </div>
                ))}
              </div>
              {/* Pays */}
              <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                {PAYS.map(({ flag, nom }) => (
                  <span key={nom} style={{ fontFamily: "var(--font-body)", fontSize: "12px", background: "#f0f7f2", color: "#1a6b3c", borderRadius: "20px", padding: "4px 12px", display: "flex", alignItems: "center", gap: "5px" }}>
                    <span>{flag}</span>{nom}
                  </span>
                ))}
              </div>
        ...
        {active === "Notre vision" && (
          <div style={{ position: "relative", borderRadius: "16px", overflow: "hidden", minHeight: "480px", display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <div style={{ position: "relative", zIndex: 1, maxWidth: "680px", margin: "0 auto", textAlign: "center", padding: "56px 40px" }}>
              {/* Guillemet décoratif */}
              <div aria-hidden="true" style={{ fontFamily: "var(--font-display)", fontSize: "80px", lineHeight: 0.6, color: "#1a6b3c", marginBottom: "28px", userSelect: "none" }}>&ldquo;</div>
              <blockquote style={{ fontFamily: "var(--font-display)", fontSize: "clamp(16px, 1.8vw, 20px)", fontWeight: 400, lineHeight: 1.75, color: "#ffffff", margin: "0 0 48px", textAlign: "justify" }}>
              ...
              <div className="piliers-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px" }}>
                {PILIERS.map(({ icon, label }) => (
                  <div key={label} style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "12px", padding: "18px 12px", backdropFilter: "blur(4px)" }}>
                    ...
                    <div style={{ fontFamily: "var(--font-body)", fontSize: "13px", fontWeight: 500, color: "rgba(255,255,255,0.85)", lineHeight: 1.3 }}>{label}</div>
                  </div>
                ))}
              </div>
            </div>
