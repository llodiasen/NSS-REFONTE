            ...
            {INFOS.map(({ Icon, label, value }, i) => (
              <div key={label}>
                <div style={{ display: "flex", alignItems: "center", gap: "16px", padding: "18px 0" }}>
                  ...
                  <div>
                    <p style={{ fontFamily: "var(--font-body)", fontSize: "10px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "1.5px", color: "rgba(255,255,255,0.4)", margin: "0 0 2px" }}>{label}</p>
                    <p style={{ fontFamily: "var(--font-body)", fontSize: "15px", color: "#ffffff", margin: 0 }}>{value}</p>
                  </div>
                </div>
        ...
        <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "16px", padding: "40px" }}>
          {success ? (
            <div style={{ textAlign: "center", padding: "32px 0" }}>
              ...
              <h3 style={{ fontFamily: "var(--font-display)", fontSize: "22px", fontWeight: 400, color: "#ffffff", marginBottom: "10px" }}>Message envoyé !</h3>
              <p style={{ fontFamily: "var(--font-body)", fontSize: "14px", color: "rgba(255,255,255,0.6)", lineHeight: 1.65 }}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} noValidate style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              {serverError && (
                <p style={{ fontFamily: "var(--font-body)", fontSize: "13px", color: "#f87171", background: "rgba(248,113,113,0.1)", padding: "10px 14px", borderRadius: "8px" }}>
                  {serverError}
                </p>
              )}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                <div>
                  <label style={{ fontFamily: "var(--font-body)", fontSize: "12px", fontWeight: 600, color: "rgba(255,255,255,0.6)", display: "block", marginBottom: "6px" }}>Prénom</label>
                  <input {...register("firstName")} placeholder="Mariama" style={INPUT} />
                  {errors.firstName && <p style={{ fontFamily: "var(--font-body)", fontSize: "11px", color: "#f87171", marginTop: "4px" }}>{errors.firstName.message}</p>}
                </div>
                <div>
                  <label style={{ fontFamily: "var(--font-body)", fontSize: "12px", fontWeight: 600, color: "rgba(255,255,255,0.6)", display: "block", marginBottom: "6px" }}>Nom</label>
                  <input {...register("name")} placeholder="Sonko" style={INPUT} />
                  {errors.name && <p style={{ fontFamily: "var(--font-body)", fontSize: "11px", color: "#f87171", marginTop: "4px" }}>{errors.name.message}</p>}
                </div>
              </div>

              {/* Email */}
              <div>
                <label style={{ fontFamily: "var(--font-body)", fontSize: "12px", fontWeight: 600, color: "rgba(255,255,255,0.6)", display: "block", marginBottom: "6px" }}>Email</label>
                <input {...register("email")} type="email" placeholder="vous@exemple.com" style={INPUT} />
                {errors.email && <p style={{ fontFamily: "var(--font-body)", fontSize: "11px", color: "#f87171", marginTop: "4px" }}>{errors.email.message}</p>}
              </div>
              ...
              {/* Message */}
              <div>
                <label style={{ fontFamily: "var(--font-body)", fontSize: "12px", fontWeight: 600, color: "rgba(255,255,255,0.6)", display: "block", marginBottom: "6px" }}>Message</label>
                <textarea {...register("message")} rows={4} placeholder="Votre message…" style={{ ...INPUT, resize: "none" }} />
                {errors.message && <p style={{ fontFamily: "var(--font-body)", fontSize: "11px", color: "#f87171", marginTop: "4px" }}>{errors.message.message}</p>}
              </div>

              {/* RGPD */}
              <label style={{ display: "flex", alignItems: "flex-start", gap: "10px", cursor: "pointer" }}>
                <input type="checkbox" {...register("rgpd")} style={{ marginTop: "3px", flexShrink: 0, accentColor: "var(--green-400)" }} />
                <span style={{ fontFamily: "var(--font-body)", fontSize: "12px", color: "rgba(255,255,255,0.45)", lineHeight: 1.5 }}>
                  J&apos;accepte que mes données soient utilisées pour traiter ma demande.
                </span>
              </label>
              {errors.rgpd && <p style={{ fontFamily: "var(--font-body)", fontSize: "11px", color: "#f87171" }}>{errors.rgpd.message}</p>}

              {/* Note + bouton */}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px", flexWrap: "wrap", marginTop: "4px" }}>
                <span style={{ fontFamily: "var(--font-body)", fontSize: "12px", color: "rgba(255,255,255,0.35)" }}>
                  ⏱ Réponse sous 48h
                </span>
                <button
                  type="submit"
