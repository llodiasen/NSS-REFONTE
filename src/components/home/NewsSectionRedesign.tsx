    ...
    <section className="nws-section" aria-labelledby="nws-titre">
      ...
      <style>{`
        ...
        @media (max-width: 1100px) {
          .nws-wrap { padding: 80px 48px; }
        }
        @media (max-width: 1024px) {
          .nws-wrap { padding: 72px 40px; }
          .nws-grid { grid-template-columns: repeat(2, 1fr); }
        }

        /* ── Mobile ── */
        @media (max-width: 768px) {
          .nws-wrap { padding: 64px 24px; }
          .nws-h2 { font-size: clamp(36px, 9vw, 54px); }
          .nws-header { margin-bottom: 48px; }
        }

        /* ── Small mobile ── */
        @media (max-width: 640px) {
          .nws-grid { grid-template-columns: 1fr; }
          .nws-wrap { padding: 56px 16px; }
          .nws-h2 { font-size: clamp(32px, 10vw, 46px); }
